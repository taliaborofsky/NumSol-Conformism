...
Load dataframe and make adjustments so it is easier to work with
...
# label by type of invasion that can happen
eqData$a_stability <- "stable"
eqData$a_stability[eqData$x_pos_invades==TRUE] <- "positive"
eqData$a_stability[eqData$x_neg_invades==TRUE] <- "negative"
eqData$a_stability <- factor(eqData$a_stability)

eqData$b_stability <- "stable"
eqData$b_stability[eqData$y_pos_invades==TRUE] <- "positive"
eqData$b_stability[eqData$y_neg_invades==TRUE] <- "negative"
...
plotC_s <- ggplot(data = datUse, mapping = aes(x = u1eq, y = u2eq, z = bueq)) + coord_tern(Tlim=c(0,1),Llim=c(0,1),Rlim=c(0,1)) + geom_point(aes(colour = x_pos_invades), size = 5, alpha = 0.8, shape = 1) + theme_bw()
...
plotC_D <- ggplot(data = datUse, mapping = aes(x = u1eq, y = u2eq, z = bueq)) + coord_tern(Tlim=c(0,1),Llim=c(0,1),Rlim=c(0,1)) + geom_point(aes(colour = y_pos_invades), size = 5, alpha = 0.8, shape = 1) + theme_bw()
...
plot_s
ggsave("Figures/ternaryAll_K.png", plot = plot_s)
